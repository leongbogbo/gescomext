...
public class CalculeCodesExportation {
	...
	public static String getCodeImportExport(String regimeCom, String CCcode, Integer numOrdre) {
		...
		} else if (deCoupeRegCommerce.length == 5) {
			List<TableauCorrespondance> tableauCorrespondanceAnc = TableauCorrespondance.getAlphaBaseDix();
			lettreAnc = deCoupeRegCommerce[3];
			for (TableauCorrespondance donneAnc : tableauCorrespondanceAnc) {
				if (donneAnc.getLettreAncien().equals(lettreAnc)) {
					keys = donneAnc.getKey();
				}
			}
	...
	public static String getCodeImportExportWithOutRCCM(String CCcode, Integer numOrdre) {
		List<TableauCorrespondance> tableauCorrespondance = TableauCorrespondance.getContribuableCode();
		char lettreCC = CCcode.substring(7, 8).charAt(0);
		// trouver la clé
		char keys = ' ';
		for (TableauCorrespondance donnees : tableauCorrespondance) {
			if (donnees.getContriKey() == lettreCC) {
	...
	public static String getLeveeGage(String usage, String chassis, String typeGage, Integer numOrdre) {
		...
		return String.valueOf(indiceAnn) + String.valueOf(lettreX) + String.valueOf(lettreT) + String.valueOf(numOrdre)
				+ String.valueOf(lettreQ);
	}
